...
int getUserByToken(char *token,int user,int *tmp_user,long long int *gold,long long int *money,long long int *house,char *name,char *url,short int *sex)
{
	int i,uid=-1;
	char *result[8];
	char *stop[32];
	...
	uid = atoi(reply->str);
	//获取ipc_user
	reply = (redisReply*)redisCommand(c,"get ipc_user_%d",uid);
	if (reply == NULL)
	{
		closeRedis();
		printf("get ipc_user redis error!\n");
		sem_post(&redis_lock);
		return -1;
	}
	...
	{
		//已登录
		*tmp_user = atoi(reply->str);
		printf("uid=%d user=%d tmp_user=%d已登录\n",uid,user,*tmp_user);
	}

	if (user != *tmp_user)
	{
		//更新ipc_user
		reply = (redisReply*)redisCommand(c,"set ipc_user_%d %d ex %d",uid,user,EX_TIME);
		if (reply == NULL)
		{
			printf("set ipc_user redis error!\n");
			closeRedis();
			sem_post(&redis_lock);
			return -1;
		}
		...
		{
			printf("set ipc_user reply error!\n");
			freeReplyObject(reply);
			//closeRedis();
			sem_post(&redis_lock);
			return -1;
...
void delIPCUser(int uid)
{
	...
	reply = (redisReply*)redisCommand(c,"del ipc_user_%d",uid);
	if (reply == NULL)
	{
		closeRedis();
		sem_post(&redis_lock);
