...
int ZJHNormalCommand(int forkid,int user,int command,char *buf)
{
	...
	switch (command)
	{
		...
		case 1007:
			//看牌
			return ZJH_check(forkid,((hall->user[user].hds)%1000)/10,(hall->user[user].hds)%10);
		case 1008:
			//上注
			recv_long64_from(buf+17,&gold);
			return ZJH_bet(forkid,((hall->user[user].hds)%1000)/10,(hall->user[user].hds)%10,gold);
		case 1009:
			//比牌
			recv_int16_from(buf+17,&parm);
			return ZJH_PK(forkid,((hall->user[user].hds)%1000)/10,(hall->user[user].hds)%10,(int)parm);
		case 1010:
			//弃牌
			return giveUp(forkid,((hall->user[user].hds)%1000)/10,(hall->user[user].hds)%10);
		case 1014:
			//桌内玩家消息
...
void ZJHNormalTimeOut(int forkid,int user,char *buf)
{
	...
	switch (hall->fork[forkid].zjh[desk].flag)
	{
		case 0:
			...
			if (hall->fork[forkid].zjh[desk].start_time != 0 && hall->time_now > hall->fork[forkid].zjh[desk].start_time)
			{
				...
				if (hall->fork[forkid].zjh[desk].turn > 0 && \
					hall->fork[forkid].zjh[desk].seat[hall->fork[forkid].zjh[desk].turn].status == 2)
				{
					//上把赢家座位准备了就当庄
				}
				else
		...
		case 1:
			...
			{
				//玩家
				giveUp(forkid,desk,hall->fork[forkid].zjh[desk].turn);
			}
			break;
	}
}
...
void zjhBalance(int forkid,int desk)
{
	...
	else
	{
		...
		if (type >= 5 && type <= 7)
		{
			...
			switch (type)
			{
				case 7:
					...
					break;
				case 6:
					prize = (rand()%20 + 11)*hall->fork[forkid].zjh[desk].bottom;
					break;
				case 5:
		...
		if (prize > 0)
		{
			...
			//生成记录
			if (add_user_prize_top(hall->user[hall->fork[forkid].zjh[desk].seat[win].user].uid,hall->fork[forkid].gameid,hall->fork[forkid].grade,hall->fork[forkid].zjh[desk].bottom,prize,type,hall->fork[forkid].zjh[desk].seat[win].card,(int)hall->time_now) < 0)
			{
				//sql error
			}
			//大奖通知
		...
		//现金场游戏记录
		if (add_game_record(hall->fork[forkid].gameid,hall->fork[forkid].type,hall->fork[forkid].grade,hall->fork[forkid].zjh[desk].bottom,\
...
int isGameOver(int forkid,int desk,int seat)
{
	...
	if (hall->fork[forkid].zjh[desk].player == 1)
	{
		...
		//更新赢家金币
		updateUserGold(hall->fork[forkid].zjh[desk].seat[win].user);
		//广播结果
...
int giveUp(int forkid,int desk,int seat)
{
	...
	//更新金币
	updateUserGold(hall->fork[forkid].zjh[desk].seat[seat].user);

	if (isGameOver(forkid,desk,seat) == 1)
	{
		//桌子信息重置
		init_desk(forkid,desk,0);
		//玩家清理
...
int ZJH_check(int forkid,int desk,int seat)
{
	...
	{
		//游戏未开始
		if (hall->fork[forkid].zjh[desk].seat[seat].card[0] != 0)
		{
...
int ZJH_PK(int forkid,int desk,int seat,int pk_seat)
{
	...
	//更新金币
	updateUserGold(hall->fork[forkid].zjh[desk].seat[lose].user);
	//
	if (isGameOver(forkid,desk,seat) == 1)
	{
		//已结束 展示牌
