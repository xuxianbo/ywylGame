	...
	switch (hall->fork[forkid].zjh[desk].flag)
	{
		case 0:
			...
			if (hall->fork[forkid].zjh[desk].start_time != 0 && hall->time_now > hall->fork[forkid].zjh[desk].start_time)
			{
				...
				hall->fork[forkid].zjh[desk].seat[0].card[1] = 3;
				hall->fork[forkid].zjh[desk].seat[0].card[2] = 2;*/
		...
		case 4:
			...
			//轮座是否出牌
			sem_wait(&(hall->fork[forkid].zjh[desk].lock));
			if (hall->fork[forkid].zjh[desk].flag != 4)
			{
				sem_post(&(hall->fork[forkid].zjh[desk].lock));
				return;
			}
...
void setMJChangeCards(char *buf,int forkid,int desk,int seat,short int *card,short int num)
{
	...
	for (i=0;i<num;i++)
	{
		printf("setMJChangeCards card = %d\n",card[i]);
	}
	for (i=0;i<num;i++)
	{
	...
	for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
	{
		if (i!=turn && hall->fork[forkid].zjh[desk].seat[i].flag_win == 3)
		{
			...
			else
			{
				...
				{
					//有天地胡规则
					if (hall->fork[forkid].zjh[desk].card_num >= 52)
					{
						for (j=0;j<hall->fork[forkid].zjh[desk].seat_max;j++)
						{
	...
	else if (flag == 0)
	{
		...
		for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
		{
			...
			else if (hall->fork[forkid].zjh[desk].seat[i].flag_gang == 3)
			{
				...
				{
					last_card = hall->fork[forkid].zjh[desk].seat[i].card_in;
					num = 1;
					...
					if (num < 4)
					{
						last_card = hall->fork[forkid].zjh[desk].seat[i].card[0];
						num = 1;
						for (j=1;j<hall->fork[forkid].zjh[desk].seat[i].card_num;j++)
						{
							if (last_card == hall->fork[forkid].zjh[desk].seat[i].card[j])
							{
								num++;
							...
							{
								last_card = hall->fork[forkid].zjh[desk].seat[i].card[j];
								num = 1;
							}
					...
					if (num < 4)
					{
						...
						//printf("card_gang=%d,card_in=%d\n",hall->fork[forkid].zjh[desk].seat[i].card_gang,hall->fork[forkid].zjh[desk].seat[i].card_in);
...
{
	short int i,flag,times,geng;
	short int check_type[8];
	short int card[20];
	...
	if (flag == 1)
	{
		//有胡
		times = 0;
		geng = check_type_mj(card,i,check_type,is_jiang(forkid,desk,seat),is_me_qing(forkid,desk,seat),is_zhong_zhang(forkid,desk,seat),is_qing(forkid,desk,seat));
		for (i=0;i<hall->fork[forkid].zjh[desk].seat[seat].num_peng;i++)
		{
			if (hall->fork[forkid].zjh[desk].seat[seat].card_peng[i] == hall->fork[forkid].zjh[desk].seat[seat].win[0].card_win)
			{
				//胡在碰上 根+1
				geng++;
				break;
		...
		if (check_type[0] == 1 && check_type[3] == 1)
		{
			//清龙七对
			times = 4;
		}
		else if (check_type[0] == 1 && check_type[2] ==1)
		{
			//清七对
			times = 4;
		}
		else if (check_type[0] == 1 && check_type[4] == 1)
		{
			//清19
			times = 4;
		}
		else if (check_type[0] == 1 && check_type[1] == 1)
		{
			//清对对胡
			times = 3;
		}
		else if (check_type[2] == 1 && check_type[6] == 1)
		{
			//将七对
			times = 4;
		}
		else if (check_type[1] == 1 && check_type[6] == 1)
		{
			//将对对胡
			times = 3;
		}
		else if (check_type[3] == 1)
		{
			//龙七对
			times = 2;
		}
		else if (check_type[2] == 1)
		{
			//七对
			times = 2;
		}
		else if (check_type[4] == 1)
		{
			//19
			times = 3;
		}
		else if (check_type[0] == 1)
		{
			//清一色
			times = 2;
		}
		else if (check_type[1] == 1)
		{
			//对对胡
			times = 1;
		}
		else if (check_type[5] == 1)
		{
			//中张
			times = 1;
		}
		else if (check_type[7] == 1)
		{
			//门清
			times = 1;
		}
...
//传递令牌
void MJPassOn(char *buf,int forkid,int desk)
{
	printf("@@@@@@@@@@@@@@@@@@@@@令牌传递!@@@@@@@@@@@@@@@@@@@@@@@\n");
	short int turn;
	int i,length;
...
{	
	...
	*times = 0;
	*geng = check_type_mj(card,num,type,is_jiang(forkid,desk,seat),is_me_qing(forkid,desk,seat),is_zhong_zhang(forkid,desk,seat),is_qing(forkid,desk,seat));
	for (i=0;i<hall->fork[forkid].zjh[desk].seat[seat].num_peng;i++)
	{
		for (j=0;j<num;j++)
		{
...
void MJBalance(int forkid,int desk)
{
	int i;
	//现金场游戏记录
	add_game_record(hall->fork[forkid].gameid,hall->fork[forkid].type,hall->fork[forkid].grade,hall->fork[forkid].zjh[desk].bottom,\
		hall->fork[forkid].zjh[desk].uid,hall->fork[forkid].zjh[desk].gold,hall->fork[forkid].zjh[desk].gold_old,hall->fork[forkid].zjh[desk].gold_new,hall->fork[forkid].zjh[desk].name,(int)(hall->time_now));
