...
int createSession(int sock,char *ip)
{
	...
	{
		//if (hall->user[i].status == 0 || (hall->user[i].status == 2 && hall->time_now - hall->user[i].time > TIME_OUT*2/10 + 2))
...
void cleanSession(int user)
{
	...
	hall->user[user].uid = -1;
	hall->user[user].socket = -1;
	hall->user[user].status = 0;
	hall->user[user].login = 0;
	hall->user[user].money = 0;
	hall->user[user].gold = 0;
	cleanIP(hall->user[user].ip);
	memset(hall->user[user].ip,'\0',IP_MAX);
	//
}
...
void cleanUser(int user)
{
	...
	//设置成断线状态
	printf("clean User uid=%d hds=%d ip=[%s]\n",hall->user[user].uid,hall->user[user].hds,hall->user[user].ip);
...
{
	if (hall->user[user].bjl_flag[0] == 0 && hall->user[user].bjl_flag[1] == 0 && hall->user[user].hds < 0)
	{
		//hall->user[user].status = 4;
		if (updateUserGold(user) == 0)
		{
			//金币更新成功
			delIPCUser(hall->user[user].uid);
			cleanSession(user);
		}
		else
		{
	...
	else if (hall->fork[forkid].gameid == 1)
	{
		...
		for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
		{
			if (hall->fork[forkid].zjh[desk].seat[i].user >= 0 && hall->user[hall->fork[forkid].zjh[desk].seat[i].user].status == 4)
			{
				...
				flushUser(hall->fork[forkid].zjh[desk].seat[i].user);
...
void OtherSeatSend(int forkid,int desk,int seat,char *buf,int length)
{
	...
	for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
	{
		if (i != seat && hall->fork[forkid].zjh[desk].seat[i].status > 0 && hall->user[hall->fork[forkid].zjh[desk].seat[i].user].status == 10)
		{
			//广播
			UserSend(hall->fork[forkid].zjh[desk].seat[i].user,buf,length);
		}
...
void AllBjlSend(int forkid,char *buf,int length)
{
	...
	for (i=0;i<USER_MAX;i++)
	{
		if (hall->user[i].hds >= 0 && hall->user[i].hds/1000 == forkid && hall->user[i].status == 10)
		{
...
int User_msg(int user,char *msg)
{
	...
	length = pack3014(buf,msg,(short)seat);
	AllSeatSend(forkid,desk,buf,length);
...
int updateUserGold(int user)
{
	...
	int ret;
	long long int gold_new = -1,money_new=-1;
	//char buf[RECV_BUF_MAX];
	sem_wait(&(hall->user[user].lock_user));
	//同步金币
	ret = update_user_gold(hall->user[user].uid,hall->user[user].gold - hall->user[user].gold_init,\
		hall->user[user].money - hall->user[user].money_init,hall->user[user].stock,hall->user[user].cake,&gold_new,&money_new);
	if (ret == -1)
	{
		//更新mysql失败
		printf("更新mysql失败\n");
	...
	else if (ret == -2)
	{
		//更新成功，获取失败
		printf("更新成功，获取失败\n");
		hall->user[user].cake = 0;
		hall->user[user].gold_init = hall->user[user].gold;
		hall->user[user].money_init = hall->user[user].money;
	}
	else if (ret == -3)
	{
		//不需要更新，不处理
	...
	else 
	{
		...
		//printf("ok!\n");
		hall->user[user].cake = 0;
		hall->user[user].gold_init = gold_new;
		hall->user[user].gold = gold_new;
		hall->user[user].money_init = money_new;
		hall->user[user].money = money_new;
		if (updateUserGoldName(hall->user[user].uid,gold_new,money_new,hall->user[user].stock,hall->user[user].name,hall->user[user].url,hall->user[user].sex) < 0)
		{
			//更新redis失败
			printf("更新redis失败\n");
			/*sprintf(buf,"updateUserGold redis updateUserGoldName error |uid=%d|money=%lld|name=%s|sex=%d|time=%d",\
				hall->user[user].uid,money_new,hall->user[user].name,hall->user[user].sex,(int)(hall->time_now));
...
int ZJH_ready(int user)
{
	...
	if ((hall->flag != 1 && hall->time_now > hall->time_over - 300) || hall->user[user].hds < 0)
	{
		//已离桌 或关服中
	...
	if (hall->fork[forkid].zjh[desk].flag == 1 || hall->fork[forkid].zjh[desk].seat[seat].status > 1)
	{
		//已准备或游戏已开始
...
int createZJHDesk(int user,short int gid,short int type,short int grade,short int flag,long long int bottom,short int ip,short int seat_max,short int round)
{
	...
	if (bottom <= 0 || (round != 0 && round != 3) || hall->user[user].money < bottom*20 || grade != PRIVATE_HOUSE || (flag != 1 && flag != 2) || (ip != 0 && ip != 1) || (seat_max != 5 && seat_max != 7))
	{
		//printf("参数错误!\n");
...
int createOXDesk(int user,short int gid,short int type,short int grade,short int flag,long long int bottom,short int ip,short int seat_max,short int model,short int kan)
{
	...
	if (bottom <= 0 || hall->user[user].money < bottom*20 || grade != PRIVATE_HOUSE || (flag != 1 && flag != 2) || (ip != 0 && ip != 1) || (seat_max != 6 && seat_max != 7) || (model != 0 && model != 1) || (kan != 0 && kan != 1))
	{
		length = pack2006(buf,gid,type,grade,-2);
		UserSend(user,buf,length);
		return 0;
...
int createMJDesk(int user,short int gid,short int type,short int grade,short int flag,long long int bottom,short int ip,short int seat_max,short int mj_change,short int mj_zm,short int mj_times,short int mj_dg,short int mj_yjjd,short int mj_mqzz,short int mj_td)
{
	...
	char buf[RECV_BUF_MAX];
	//printf("uid %d money=%lld createDesk,gid=%d,type=%d,grade=%d,flag=%d,bottom=%lld,ip=%d,seat_max=%d,round=%d\n",hall->user[user].uid,hall->user[user].money,gid,type,grade,flag,bottom,ip,seat_max,round);
	if (bottom <= 0 || hall->user[user].money < bottom*20 || grade != PRIVATE_HOUSE || (flag != 1 && flag != 2) || (seat_max != 3 && seat_max != 4) || mj_times < 2 || mj_times > 12)
	{
		//printf("参数错误!\n");
		length = pack2006(buf,gid,type,grade,-2);
		UserSend(user,buf,length);
...
int ZJH_inDesk(int user,int forkid,char *token,int hds)
{
	...
	//printf("入桌 over ! fork%d desk%d seat%d uid=%d\n",forkid,desk,seat,hall->user[user].uid);
	//入桌应答
	length = pack2003(buf,1,forkid,desk,(short)seat);
	UserSend(user,buf,length);
...
int getUserGoldFromRedis(int user,int type)
{
	...
	sem_wait(&(hall->user[user].lock_user));
	if (type==1 && getGoldNameByUid(hall->user[user].uid,&gold,&money,&stock,hall->user[user].name,hall->user[user].url,&(hall->user[user].sex)) >= 0)
	{
		//printf("gold_init=%lld,gold=%lld\n",hall->user[user].gold_init,gold);
		if (hall->user[user].gold_init != gold)
		{
			hall->user[user].gold += (gold - hall->user[user].gold_init);
			hall->user[user].gold_init = gold;
		}

		if (hall->user[user].money_init != money)
		{
			hall->user[user].money += (money - hall->user[user].money_init);
