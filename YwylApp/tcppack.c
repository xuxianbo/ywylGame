...
int pack2001(char *buf,short int flag,short int ret,short int gid,short int type,short int grade,int hds)
{
	char *p;
	p = buf;
	*p = '|';
...
int pack2018(char *buf,short int gid,short int type)
{
	...
	*p = '|';
	...
	for (i=0;i<hall->normal_num;i++)
	{
		if (hall->fork[i].gameid == gid && hall->fork[i].type == type && hall->fork[i].grade == PRIVATE_HOUSE)
		{
...
{
	int i,j,length = 15,name_len,url_len,seat_num,tmp_user;
	short int sex=0;
	char name[32]="";
	char url[64]="";
	char *p;
	...
	*p = '|';
	...
	length += (url_len+name_len+24+bet_max*8);
	//入座闲家信息
	seat_num = 1;
	for (i=0;i<look_max;i++)
	{
		...
		p+=url_len;
		length += (url_len+name_len+22+bet_max*8);
	}
	send_int16_2buf(p,hall->fork[forkid].gameid);
	p+=2;
	...
	*p = '|';
	...
	if (seat < 0)
	{
		...
		for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
		{
			...
			//
			length += (length_name+url_len+ip_len+42+hall->fork[forkid].zjh[desk].seat[i].card_out_num*2+(hall->fork[forkid].zjh[desk].seat[i].num_peng+hall->fork[forkid].zjh[desk].seat[i].num_gang)*4);
	...
	else
	{
		...
		length += (length_name+url_len+ip_len+42+hall->fork[forkid].zjh[desk].seat[seat].card_out_num*2+(hall->fork[forkid].zjh[desk].seat[seat].num_peng+hall->fork[forkid].zjh[desk].seat[seat].num_gang)*4);
	}
