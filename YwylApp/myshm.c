...
//	printf("%d,%d\n",i_shm_key,ShmSize);
	...
	if(i_flag == 1)
	{
		...
		for (i=0;i<MSG_LIST_MAX;i++)
		{
			...
			hall->aaa_msg[i].id = 0;
			memset(hall->aaa_msg[i].msg,'\0',128);
		}
		//
		for(i=0;i<FORK_MAX;i++)
		{
			...
			for (j=0;j<DESK_MAX;j++)
			{
				...
				hall->fork[i].zjh[j].mj_change = 0;
				hall->fork[i].zjh[j].mj_times = 3;
				...
				hall->fork[i].zjh[j].model = 0;
				hall->fork[i].zjh[j].kan = 0;
			}
		}
		//百家乐
		for (m=0;m<2;m++)
...
void init_desk(int forkid,int desk,int flag)
{
	...
	if (flag)
	{
		...
		//hall->fork[forkid].zjh[desk].ip = 0;
		//hall->fork[forkid].zjh[desk].model = 0;
		//hall->fork[forkid].zjh[desk].kan = 0;
