...
#define SHM_KEY 2018125
#define FORK_MAX 0+HB_MAX*10		//房间数量上限
...
#define PASSWD_MAX	32				//用户密码长度
#define AGENT_MAX 5					//代理层级上限
#define GROUP_MAX 20				//入群数量上限
...
#define BJL_GAME_MAX 5				//百家乐游戏数量
#define BJL_HOUSE_MAX 10			//百家乐房间上限
#define BJL_HAND_MAX 5				//百家乐手牌上限
#define BANKER_MAX 20				//百家乐上庄队列上限
#define BJL_BET_MAX	8				//百家乐投注区域数量上限
#define BJL_L_MAX 30				//百家乐历史开奖记录
#define BJL_SEAT_MAX 8				//百家乐可视桌位数
#define MSG_LIST_MAX 20				//百家乐同时聊天上限
#define WINNER_MAX	5				//百家乐大赢家列表上限

#define HB_MAX 10					//红包队列类型总数
#define HB_LIST_MAX 100				//红包进行队列上限
#define USER_CATCH_MAX 10			//抢红包人数上限

#define ORDER_MAX 10				//订单并发上限
#define MSG_BUF_MAX 2000			//聊天消息最大长度
...
{	
	...
	short int no_flag[BJL_BET_MAX];			//必不开次数
	...
	long long int robot_gold[BJL_BET_MAX];	//机器人下注
...
//红包
//抢到红包的用户队列
struct HB_USER_CATCH
{			
	short int notice;				//是否需要通知发包人 0 不需要 1 需要
	char name[USER_NAME_MAX];
	int uid;
	int catch_time;					//抢到时间
	long long int gold;				//抢到的红包数额
	long long int gain;				//抢红包净额
	long long int lose;				//抢红包踩雷输分数
	long long int code;				//特码中奖情况
};
typedef struct HB_USER_CATCH HB_USER_CATCH;
...
{	
	...
	int user;
	int banker_uid;
	char banker_name[USER_NAME_MAX];
	char banker_url[USER_HEAD_URL];
	long long int banker_gold;		//红包数额
	long long int banker_gain;		//发红包者净额
	long long int banker_win;		//发红包者赢雷分数
	long long int banker_back;		//退款情况
	long long int banker_code;		//特码中奖情况
	short int mine;					//雷号
	short int mine_num;				//雷个数
	short int status;				//状态 0无红包 1红包进行中 2 红包已结算 3已全部通知发包人
	time_t begin_time;				//开抢时间
	
	short int catch_num;			//已抢到的用户数
	HB_USER_CATCH user_catch[USER_CATCH_MAX]; //抢到红包的用户队列
};
...
{	
	int group_id;							//群id
	short int flag;							//群状态 0正常 1冻结
	short int house_num;					//房间总数

	long long int bottom;					//每个红包的底注
	short int player;						//每个红包，最大可抢人数

	short int list_head;					//红包头部	
	HB_LIST  list[HB_LIST_MAX];				//红包进行队列
	
	sem_t lock_msg;							//聊天锁
	short int msg_list_flag;				//聊天列表状态,-1无数据，0有数据可写入，1不可写入
	BJL_MSG hb_msg[MSG_LIST_MAX];			//聊天列表
	short int flag_msg[BJL_HOUSE_MAX];		//聊天通知
	short int flag_start[BJL_HOUSE_MAX][HB_LIST_MAX];  //红包可抢通知
	short int flag_over[BJL_HOUSE_MAX][HB_LIST_MAX];  //红包结算通知
};
...
{				
	...
	//bjl
	short int banker_flag[BJL_GAME_MAX];				//庄家状态，0初始化，1上庄等待中，2我要下庄，10上庄中
	short int bjl_flag[BJL_GAME_MAX];					//百家乐是否下注 0 没有或已结算，1 已下注  
	short int bjl_balance[BJL_GAME_MAX];				//百家乐是否结算通知 0 不需要通知 1 需要通知
	long long int bet[BJL_GAME_MAX][BJL_BET_MAX];		//各方下注总额
	long long int gold_win[BJL_GAME_MAX][BJL_BET_MAX];	//各方赢金数

	//红包
	short int hb_send_num[HB_MAX];					//发红包的数量(未结算)		
	...
	//群id列表
	short int group_num;				//群数量
	short int group[GROUP_MAX];			//群id列表
	//上级关系
	short int agent_num;				//上级代理个数
	int agent[AGENT_MAX];				//上级代理uid
	
	//聊天缓存
	sem_t lock_user_msg;				//个人消息锁
	short int user_msg_num;				//个人消息条数
	AAA_MSG user_msg[MSG_LIST_MAX];		//个人消息队列
};
