			...
			if (hall->fork[forkid].zjh[desk].start_time != 0 && hall->time_now > hall->fork[forkid].zjh[desk].start_time)
			{
				...
				for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
				{
					...
					if (hall->fork[forkid].zjh[desk].seat[i].status == 2)
					{
						...
						j = check_type_ox(hall->fork[forkid].zjh[desk].seat[i].card,hall->fork[forkid].zjh[desk].model,hall->fork[forkid].zjh[desk].kan);
						if ((j == 15 || j== 17) && card_num >= 5*(hall->fork[forkid].zjh[desk].seat_max-i))
						{
							getMoreCards(1,5,cards,&card_num,hall->fork[forkid].zjh[desk].seat[i].card);
						}
					...
					{
						case 0:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 36;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 22;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 8;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 21;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 7;
							break;
						case 1:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 13;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 12;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 11;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 10;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 6;
							break;
						case 2:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 16;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 2;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 42;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 28;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 14;
							break;
						case 3:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 27;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 26;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 25;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 24;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 23;
							break;
						case 4:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 13;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 12;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 25;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 10;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 9;
							break;
						case 5:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 41;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 27;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 40;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 39;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 25;
							break;
						case 6:
							hall->fork[forkid].zjh[desk].seat[i].card[0] = 4;
							hall->fork[forkid].zjh[desk].seat[i].card[1] = 45;
							hall->fork[forkid].zjh[desk].seat[i].card[2] = 31;
							hall->fork[forkid].zjh[desk].seat[i].card[3] = 17;
							hall->fork[forkid].zjh[desk].seat[i].card[4] = 3;
							break;
					}*/
				}
...
int OXCommand(int forkid,int user,int command,char *buf)
{
	...
	switch (command)
	{
		...
		case 1008:
			...
			return OX_bet(buf,forkid,((hall->user[user].hds)%1000)/10,(hall->user[user].hds)%10,gold);
		case 1040:
			//牛牛开牌
...
void OX_balance(int forkid,int desk,int seat,char *buf)
{
	...
	long long int gold_win,gold_lost,gold_tmp;
	long long int gold_balance[SEAT_MAX];
	int times,banker;
	...
	else
	{
		...
		for (i=0;i<hall->fork[forkid].zjh[desk].seat_max;i++)
		{
			...
			if (hall->fork[forkid].zjh[desk].seat[i].status == 6)
			{
				...
				if (type >= 11)
				{
					while (add_user_prize_top(hall->user[hall->fork[forkid].zjh[desk].seat[i].user].uid,hall->fork[forkid].gameid,hall->fork[forkid].grade,hall->fork[forkid].zjh[desk].bottom,prize,type,hall->fork[forkid].zjh[desk].seat[i].card,(int)hall->time_now) < 0)
					{
						//sql error
						sleep(1);
					}
		...
		//writeToFile(file,buf);
		while (add_game_record(hall->fork[forkid].gameid,hall->fork[forkid].type,hall->fork[forkid].grade,hall->fork[forkid].zjh[desk].bottom,hall->fork[forkid].zjh[desk].uid,gold_balance,hall->fork[forkid].zjh[desk].gold_old,hall->fork[forkid].zjh[desk].gold_new,hall->fork[forkid].zjh[desk].name,(int)(hall->time_now)) < 0)
		{
			//sql error
			sleep(1);
		}
...
int OX_bet(char *buf,int forkid,int desk,int seat,long long int gold)
{
	...
	switch (hall->fork[forkid].zjh[desk].flag)
	{
		case 1:
			...
			if (hall->fork[forkid].type == 3)
			{
				if (hall->fork[forkid].zjh[desk].seat[seat].status != 3 || gold < 0 || gold > 2)
				{
					return 0;
				}
				gold_max = gold_user/(hall->fork[forkid].zjh[desk].bottom*10*(hall->fork[forkid].zjh[desk].player-1));
			}
			else
			{
		...
		case 2:
			...
			if (hall->fork[forkid].type == 3)
			{
				if (hall->fork[forkid].zjh[desk].seat[seat].status != 4 || gold < 1 || gold > 3)
				{
					return 0;
				}
				gold_max = gold_user/(hall->fork[forkid].zjh[desk].bottom*10*hall->fork[forkid].zjh[desk].in_total);
			}
			else
			{
